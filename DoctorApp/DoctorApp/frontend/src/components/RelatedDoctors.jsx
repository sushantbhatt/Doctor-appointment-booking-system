    ...
    <div className="flex flex-col items-center gap-6 my-16 text-gray-900 md:mx-10">
      ...
      <div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 pt-5 gap-y-6 px-3 sm:px-0">
        {relDoc.slice(0, 5).map((doctor, index) => (
          <button 
            key={index} 
            onClick={() => navigate(`/appointment/${doctor._id}`)}  // Using navigate to redirect
      ...
      <button 
        onClick={() => { navigate('/doctors'); scrollTo(0, 0); }}  // Navigate to doctors page
        className="bg-blue-200 text-gray-600 px-12 py-3 rounded-full mt-10 hover:bg-blue-300 transition"
      >
        More
      </button>
    </div>
